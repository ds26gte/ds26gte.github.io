...
$ X = mat(delim: "[", 1, x; dot, dot; dot, dot; dot, dot) ,
  Y = mat(delim: "[", y; dot; dot; dot) $

Using the multiple-regression algorithm, we get

$ B = vec(delim: "[", alpha, beta) = (X^T X)^(-1) X^T Y.
quad quad (1) $
...
$ X^T = mat(delim: "[", 1, dot, dot, dot; x, dot, dot, dot) $

$ therefore X^T X = mat(delim: "[", 1, dot, dot, dot; x, dot, dot, dot)
                    mat(delim: "[", 1, x; dot, dot; dot, dot; dot, dot) = 
                    mat(delim: "[", n, Sigma x; Sigma x, Sigma x^2) $
...
"and" quad quad "cof" X^T X &= 
   mat(delim: "[", Sigma x^2, - Sigma x; - Sigma x, n) $
...
(1/Delta) mat(delim: "[", Sigma x^2, - Sigma x; - Sigma x, n) $

Putting all this in (1), we have

$ B &= (1/Delta) mat(delim: "[", Sigma x^2, -Sigma x; -Sigma x, n)
mat(delim: "[", 1, dot, dot, dot; x, dot, dot, dot)
mat(delim: "[", y; dot; dot; dot) \ 

    &= (1/Delta) mat(delim: "[", Sigma x^2, -Sigma x; -Sigma x, n)
    mat(delim: "[", Sigma y; Sigma x y) \

    &= (1/Delta)
    mat(delim: "[", Sigma x^2 Sigma y - Sigma x Sigma x y;
